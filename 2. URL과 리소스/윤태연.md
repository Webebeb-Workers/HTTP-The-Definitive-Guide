...
> [!NOTE]
> 인터넷상의 모든 리소스를 가리키는 표준화된 이름, 사용자-브라우저가 정보를 찾는데 필요한 리소스의 위치
...
> [!NOTE]
> 특정 리소스에 접근하기 위해 필요한 추가 정보
> 애플리케이션이 서버에 정확한 요청을 하기 위해 필요한 입력 파라미터를 받는데 사용
> `/`로 경로 조각을 구분하며 `;`로 파라미터 추가
...
> [!NOTE]
> 애플리케이션에 추가 데이터를 전달
> DB 등의 서비스는 요청 리소스 형식의 범위를 좁히기 위해서 질의 사용
> `?` 이후 `키=값`의 형태로 쓰이고, `&`로 구분
...
> [!NOTE]
> 현재 문서의 URL을 기준으로 리소스의 위치를 지정하는 방식
> 현재 절대 URL을 추출한 기저 URL을 이용해 주소창에 상태 URL을 입력해도 해당 파일로 갈 수 있음
...
> [!NOTE]
> 안전하지 않은 문자를 `%`를 시작으로, 두 개의 16진수로 인코딩 => 퍼센트(%) 인코딩, '이스케이프' 문자로 변환
